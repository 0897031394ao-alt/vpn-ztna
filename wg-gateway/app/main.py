...
def ensure_wg_interface() -> None:
    ...
    if check.returncode != 0:
        create = run_cmd(["ip", "link", "add", "dev", WG_INTERFACE, "type", "wireguard"])
        if create.returncode != 0:
            raise RuntimeError(f"cannot create {WG_INTERFACE}: {create.stderr.strip()}")

    addr_show = run_cmd(["ip", "address", "show", "dev", WG_INTERFACE])
    if WG_ADDRESS.split("/")[0] not in addr_show.stdout:
        addr_add = run_cmd(["ip", "address", "add", WG_ADDRESS, "dev", WG_INTERFACE])
        if addr_add.returncode != 0 and "File exists" not in addr_add.stderr:
            raise RuntimeError(f"cannot assign address: {addr_add.stderr.strip()}")

    if not os.path.exists(WG_PRIVATE_KEY_FILE):
        raise RuntimeError(f"private key file not found: {WG_PRIVATE_KEY_FILE}")

    wg_set = run_cmd(
        ["wg", "set", WG_INTERFACE, "private-key", WG_PRIVATE_KEY_FILE, "listen-port", str(WG_PORT)]
    )
    if wg_set.returncode != 0:
        raise RuntimeError(f"cannot configure {WG_INTERFACE}: {wg_set.stderr.strip()}")

    link_up = run_cmd(["ip", "link", "set", "up", "dev", WG_INTERFACE])
    if link_up.returncode != 0:
        raise RuntimeError(f"cannot bring up {WG_INTERFACE}: {link_up.stderr.strip()}")
...
@app.get("/health")
def health():
    return {"status": "ok", "interface": WG_INTERFACE}
...
@app.post("/wg/peers/apply")
def apply_peer(payload: ApplyPeerRequest):
    cmd = [
        "wg", "set", WG_INTERFACE,
        "peer", payload.public_key,
        "allowed-ips", payload.allowed_ips,
    ]
    ...
    if payload.persistent_keepalive is not None:
        cmd += ["persistent-keepalive", str(payload.persistent_keepalive)]
